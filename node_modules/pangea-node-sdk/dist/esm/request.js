...
import { ConfigEnv, TransferMethod, } from "./types.js";
...
class PangeaRequest {
    ...
    async post(endpoint, data, options = {}) {
        ...
        if (options.files && entry) {
            ...
            const responseType = data.transfer_method === TransferMethod.MULTIPART ? "buffer" : "json";
            const request = {
    ...
    async downloadFile(url) {
        ...
        return new AttachedFile(filename, await response.arrayBuffer(), contentType);
    }
    ...
    async postPresignedURL(url, fileData) {
        ...
        if (!response.ok) {
            throw new PangeaErrors.PresignedUploadError(`presigned POST failure: ${response.status}`, JSON.stringify(await response.json()));
        }
    }
    async putPresignedURL(url, fileData) {
        ...
        if (!response.ok) {
            throw new PangeaErrors.PresignedUploadError(`presigned PUT failure: ${response.status}`, JSON.stringify(await response.json()));
        }
        return;
    }
    ...
    async httpPost(url, options) {
        ...
        this.checkResponse(new PangeaResponse(response, await response.arrayBuffer()));
    ...
    async httpRequest(url, options) {
        ...
        return await promiseRetry(async (retry, _attempt) => {
    ...
    async handleHttpResponse(response, options = {}) {
        let pangeaResponse = response instanceof PangeaResponse
            ? response
    ...
    async get(endpoint, checkResponse = true) {
        ...
        const pangeaResponse = new PangeaResponse(response, await response.arrayBuffer());
    ...
    checkResponse(response) {
        ...
        switch (response.status) {
            ...
            case "NotFound":
                throw new PangeaErrors.NotFound(response.gotResponse?.url !== undefined

...
class AuditService extends base_js_1.default {
    ...
    async logBulkAsync(events, options = {}) {
        ...
        try {
            ...
            if (e instanceof errors_js_1.PangeaErrors.AcceptedRequestException) {
                // TODO: bad type cast
    ...
    getLogEvent(event, options) {
        ...
        event = (0, utils_js_1.eventOrderAndStringifySubfields)(event);
        const data = { event: event };
        if (options.signer) {
            const signer = options.signer;
            const signature = signer.sign((0, utils_js_1.canonicalizeEvent)(event));
    ...
    processLogResponse(result, options) {
        ...
        if (!options?.skipEventVerification) {
            ...
            result.signature_verification = (0, verification_js_1.verifySignature)(result.envelope);
        }
        if (options?.verify) {
            result.membership_verification = (0, verification_js_1.verifyLogMembershipProof)({
                log: result,
                newUnpublishedRootHash: newUnpublishedRootHash,
            });
            result.consistency_verification = (0, verification_js_1.verifyLogConsistencyProof)({
                log: result,
    ...
    async processSearchResponse(response, options) {
        ...
        if (options?.verifyConsistency) {
            ...
            if (root !== undefined) {
                ...
                this.publishedRoots = await (0, arweave_js_1.getArweavePublishedRoots)(treeName, Array.from(treeSizes), localRoot);
            }
            const pending = [];
            response.result.events.forEach((record) => {
